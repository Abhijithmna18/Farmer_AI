...
export default function SoilRecords(){
  ...
  const create = async (e) => {
    e.preventDefault();
    await soilService.create({ ...form, sampleDate: new Date(form.sampleDate) });
  ...
  return (
    <div className="p-4">
      ...
      <form onSubmit={create} className="border rounded p-3 mb-4 grid gap-2">
        ...
        <input className="border px-3 py-2 rounded" placeholder="pH" value={form.ph} onChange={(e)=>setForm({...form, ph: e.target.value})} />
        <input className="border px-3 py-2 rounded" placeholder="Nitrogen (ppm)" value={form.nitrogen} onChange={(e)=>setForm({...form, nitrogen: e.target.value})} />
        <input className="border px-3 py-2 rounded" placeholder="Phosphorus (ppm)" value={form.phosphorus} onChange={(e)=>setForm({...form, phosphorus: e.target.value})} />
        <input className="border px-3 py-2 rounded" placeholder="Potassium (ppm)" value={form.potassium} onChange={(e)=>setForm({...form, potassium: e.target.value})} />
